...
export async function createProjectStructure(rootPath: string): Promise<void> {
  const folders = [
    "lib/ui/core/ui",
    "lib/ui/core/themes",
    "lib/domain/models",
...
export async function createFeatureInPresentationLayer(
  rootPath: string,
  featureName: string
): Promise<void> {
  ...
  const viewModelContent = `import 'package:flutter/material.dart';
...
export async function createFeatureInDataLayer(
  rootPath: string,
  featureName: string
): Promise<void> {
  ...
  const repositoryContent = `/// Data source for activities.
abstract class ${className}Repository {
  /// Add the interface methods here
}
`;

  const localRepositoryContent = `import '${featureName}_repository.dart';
...
`;
  ...
  const remoteRepositoryContent = `import '${featureName}_repository.dart';
...
export async function createFeatureInDomainLayer(
  rootPath: string,
  featureName: string
): Promise<void> {
  ...
  const modelContent = `class ${className} {
...
export async function createFeatureInAllLayers(
  rootPath: string,
  featureName: string
): Promise<void> {
...
export async function createBaseConfiguration(rootPath: string): Promise<void> {
  ...
  const dependenciesContent = `import 'package:provider/single_child_widget.dart';
...
`;
  ...
  const assetsContent = `abstract final class Assets {
...
export async function createBaseRouting(rootPath: string): Promise<void> {
  ...
  const routerContent = `//! Add your router configuration here
`;

  const routerFile = path.join(rootPath, `lib/routing/router.dart`);

  const routesContent = `abstract final class Routes {
  static const home = '/';
